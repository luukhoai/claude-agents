...
def test_update_contact_success(client):
    """Test PUT /api/contacts/{id} to update contact name."""
    # First create a contact
    response = client.post('/api/contacts', json={
        'name': 'John Doe',
        'email': 'john@example.com',
    ...
    assert data['data']['email'] == 'john@example.com'  # Email unchanged


def test_update_contact_not_found(client):
    """Test PUT /api/contacts/{id} with non-existent ID returns 404."""
    response = client.put('/api/contacts/nonexistent-id', json={
        'name': 'New Name'
    })
...
def test_update_contact_empty_name(client):
    """Test PUT /api/contacts/{id} with empty name returns 400."""
    # First create a contact
    response = client.post('/api/contacts', json={
        'name': 'John Doe',
        'email': 'john@example.com',
...
def test_update_contact_email_ignored(client):
    """Test PUT /api/contacts/{id} ignores email field in request body."""
    # First create a contact
    response = client.post('/api/contacts', json={
        'name': 'John Doe',
        'email': 'john@example.com',
    ...
    # Try to update with new email - it should be ignored
    response = client.put(f'/api/contacts/{contact_id}', json={
        'name': 'John Smith',
    ...
    assert data['success'] is True
    assert data['data']['name'] == 'John Smith'
    assert data['data']['email'] == 'john@example.com'  # Email should remain unchanged


def test_update_contact_no_data(client):
    """Test PUT /api/contacts/{id} with no data returns 400."""
    # First create a contact
    response = client.post('/api/contacts', json={
        'name': 'John Doe',
        'email': 'john@example.com',
