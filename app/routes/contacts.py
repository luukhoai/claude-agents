...
@contacts_bp.route('/by-email', methods=['DELETE'])
def delete_contact_by_email():
    """
    Delete a contact by email address.

    Query Parameters:
        email: Email address of the contact to delete (required)
    ...
    try:
        ...
        current_app.logger.info(
            f"Contact deleted: email={email}, id={contact['id']}")
...
def update_contact(contact_id):
    ...
    Only the 'name' field can be updated. Email changes are not allowed.

    Path Parameters:
        contact_id: ID of the contact to update

    Request Body:
        name: New name (required, cannot be empty)
        email: If provided, will be ignored (email changes not allowed)

    Returns:
        JSON response with updated contact or error
    """
    try:
        data = request.json
        if not data:
        ...
        # Get the name from request - this is the only field we allow to update
        name = data.get('name', '').strip() if data.get('name') else ''

        # Validate name is provided and not empty
        if not name:
            return jsonify({'success': False, 'error': 'Name is required and cannot be empty'}), 400
        ...
        # Update only the name field (ignore email if provided)
        # Email changes are not allowed as per requirements
        old_name = contact['name']
        contact['name'] = name

        current_app.logger.info(
            f"Contact {contact_id} updated: name changed from '{old_name}' to '{name}'"
        )
    ...
    try:
        ...
        if field not in valid_fields:
            return jsonify({'success': False,
                            'error': f'Invalid field. Must be one of: {", ".join(valid_fields)}'}), 400

        valid_sort_fields = ['created_at', 'name', 'email']
        if sort_field not in valid_sort_fields:
            return jsonify({'success': False,
                            'error': f'Invalid sort field. Must be one of: {", ".join(valid_sort_fields)}'}), 400
